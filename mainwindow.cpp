...
{
     ui->tabWidget->addTab(new MyCodeEditor(this,QFont(mFontFamily,mFontSize)),"Newfile.zp");
     initAction();
}
...
{
    MyCodeEditor * codeEditor = (MyCodeEditor*)ui->tabWidget->widget(index);
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    if(ui->tabWidget->count()>0){
        QMessageBox::question(this,"警告","有未保存的文件， 确定退出吗? "
